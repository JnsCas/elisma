...
export class ZipController {
  ...
  async generate(req: GenerateZipRequest, res: FastifyReply): Promise<void> {
    ...
    const supportedLibraries = await this.manifestService.loadManifests()

    const libs = selectedLibraries.map(
      (selectedLib) => supportedLibraries.find((supportedLib) => selectedLib.name === supportedLib.name) as LibManifest
    )
    ...
    const manifests = await this.manifestService.findManifests(libs)
